...
def build_block(manifest: dict) -> str:
  ...
  <style>
    ...
    #moonn-review-funnel.is-submitted .rf-shell{{grid-template-columns:minmax(0,680px);justify-content:center;}}
    ...
    #moonn-review-funnel.is-submitted .rf-intro,#moonn-review-funnel.is-submitted .rf-form>h3,#moonn-review-funnel.is-submitted .rf-form>.rf-offer-note,#moonn-review-funnel.is-submitted .rf-form>form,#moonn-review-funnel.is-submitted .rf-form>.rf-status{{display:none!important;}}
    #moonn-review-funnel.is-submitted .rf-form{{padding:0;background:transparent;box-shadow:none;border:0;}}
    ...
    #moonn-review-funnel .rf-rating-done{{display:none;margin-top:18px;padding:16px;border-radius:16px;background:#f4fff8;border:1px solid rgba(22,128,80,.18);color:#20573c;}}
    ...
    #moonn-review-funnel.is-submitted .rf-participant-offer{{margin-top:0;padding:24px;}}
    #moonn-review-funnel .rf-participant-offer.is-visible{{display:block;}}
    #moonn-review-funnel .rf-participant-offer h4{{margin:0 0 8px;color:#181c20;font-size:24px;line-height:1.2;}}
    ...
    #moonn-review-funnel .rf-copy-message{{width:100%;min-height:78px;margin-top:12px;padding:12px;border:1px solid rgba(31,35,40,.14);border-radius:14px;background:#fff;color:#303841;font:500 13px/1.45 Arial,sans-serif;resize:vertical;}}
    #moonn-review-funnel .rf-proof-card{{display:none;margin-top:14px;padding:12px;border-radius:16px;background:#fff;border:1px solid rgba(31,35,40,.12);}}
    #moonn-review-funnel .rf-proof-card.is-visible{{display:block;}}
    #moonn-review-funnel .rf-proof-card strong{{display:block;margin-bottom:6px;color:#181c20;font-size:15px;}}
    #moonn-review-funnel .rf-proof-card span{{display:block;color:#687583;font-size:13px;line-height:1.4;}}
    #moonn-review-funnel .rf-proof-preview{{display:block;width:min(100%,260px);max-height:330px;object-fit:contain;margin:12px auto 0;border-radius:12px;border:1px solid rgba(31,35,40,.12);background:#fff;box-shadow:0 8px 18px rgba(24,39,75,.10);}}
  ...
  <div class="rf-shell">
    <article class="rf-card rf-intro">
      ...
      <p>Вы можете поставить оценку на Яндекс Услугах, нажав жёлтую кнопку, или оставить текстовый отзыв для сайта Moonn.ru.</p>
      <div class="rf-person">
    ...
    <article class="rf-card rf-form">
      ...
      <div class="rf-participant-offer" data-participant-offer aria-live="polite">
        ...
        <p>Отзыв опубликован на странице Татьяна Мун.ру. Ниже уже подготовлены текст и подтверждение отзыва для сообщения Татьяне в Telegram.</p>
        <textarea class="rf-copy-message" data-offer-message readonly></textarea>
        <div class="rf-proof-card" data-proof-card>
          <strong>Подтверждение отзыва</strong>
          <span>Эту картинку можно отправить Татьяне вместе с сообщением.</span>
          <img class="rf-proof-preview" data-proof-preview alt="Подтверждение опубликованного отзыва Moonn" hidden>
        </div>
        <div class="rf-actions">
          <button class="rf-button primary" type="button" data-copy-open-telegram>Скопировать текст и открыть Telegram Татьяны</button>
          <button class="rf-button ghost" type="button" data-share-proof hidden>Отправить текст и подтверждение в Telegram</button>
          <a class="rf-button ghost" data-booking-link href="{escape_html(YCLIENTS_BOOKING_URL)}" target="_blank" rel="noopener noreferrer">Записаться онлайн</a>
          <a class="rf-button ghost" href="https://moonn.ru/otzivi">Закрыть страницу</a>
        </div>
      </div>
    </article>
  ...
  <script>
    (function(){{
      ...
      var copyOpenTelegramButton = root.querySelector('[data-copy-open-telegram]');
      var proofCard = root.querySelector('[data-proof-card]');
      var proofPreview = root.querySelector('[data-proof-preview]');
      var shareProofButton = root.querySelector('[data-share-proof]');
      var bookingLink = root.querySelector('[data-booking-link]');
      var lastProofFile = null;
      ...
      function jsonp(params, onDone) {{
        var callbackName = '__moonnReviewsCb' + Date.now() + Math.floor(Math.random() * 100000);
        ...
        script.onerror = function() {{
          ...
          delete window[callbackName];
          if(script.parentNode) script.parentNode.removeChild(script);
          onDone(new Error('network_error'));
        }};
      ...
      function buildOfferMessage(review) {{
        ...
        var parts = [
          ['Здравствуйте,', 'Татьяна.'].join(' '),
          ['Я оставил(а) отзыв на сайте Moonn.ru.', 'Прошу зафиксировать специальное условие участника мероприятия Moonn на первое двухчасовое посещение для меня или близкого.'].join(' '),
          'Прикрепляю подтверждение отзыва с сайта Moonn.ru.',
          fieldLine('Имя для публикации', review && review.namePublic ? review.namePublic : 'не указано'),
          fieldLine('Что посетил(а)', review && review.context ? review.context : 'не указано'),
          fieldLine('ID отзыва', review && review.id ? review.id : 'не указан')
        ];
        return parts.join('\\n');
      }}
      ...
      function buildReviewProofImage(review) {{
        ...
        if(!ctx) return null;
        var created = review.createdAt ? new Date(review.createdAt) : new Date();
        var createdText = isNaN(created.getTime()) ? new Date().toLocaleDateString('ru-RU') : created.toLocaleDateString('ru-RU');
        ctx.fillStyle = '#f4f0ff';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        var gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
        gradient.addColorStop(0, '#ffffff');
        gradient.addColorStop(0.62, '#ffffff');
        gradient.addColorStop(1, '#fff6cf');
        ctx.fillStyle = gradient;
        roundRect(ctx, 70, 70, 940, 1360, 34, true, false);
        ctx.strokeStyle = '#ded2fb';
        ctx.lineWidth = 3;
        roundRect(ctx, 70, 70, 940, 1360, 34, false, true);
        ctx.fillStyle = '#ffcc00';
        roundRect(ctx, 110, 120, 84, 84, 22, true, false);
        ctx.fillStyle = '#111111';
        ctx.font = '700 48px Arial';
        ctx.fillText('Я', 137, 178);
        ctx.fillStyle = '#181c20';
        ctx.font = '700 44px Arial';
        ctx.fillText('Подтверждение отзыва', 220, 152);
        ctx.font = '400 27px Arial';
        ctx.fillStyle = '#4d5965';
        ctx.fillText('Moonn.ru · Татьяна Мунн', 220, 192);
        ctx.fillStyle = '#5b2ec7';
        ctx.font = '700 38px Arial';
        ctx.fillText('Отзыв опубликован на сайте Moonn.ru', 110, 280);
        ctx.fillStyle = '#303841';
        ctx.font = '400 27px Arial';
        wrapCanvasText(ctx, 'Прошу зафиксировать специальное условие участника мероприятия Moonn на первое двухчасовое посещение для меня или близкого.', 110, 330, 840, 38, 3);
        ctx.fillStyle = '#181c20';
        ctx.font = '700 32px Arial';
        ctx.fillText('Имя', 110, 470);
        ctx.font = '400 34px Arial';
        ctx.fillText(review.namePublic || 'не указано', 110, 515);
        ctx.font = '700 32px Arial';
        ctx.fillText('Что посетил(а)', 110, 600);
        ctx.font = '400 34px Arial';
        ctx.fillText(review.context || 'не указано', 110, 645);
        ctx.font = '700 32px Arial';
        ctx.fillText('Оценка', 110, 730);
        ctx.fillStyle = '#f5b700';
        ctx.font = '700 46px Arial';
        ctx.fillText('★ ★ ★ ★ ★', 110, 785);
        ctx.fillStyle = '#181c20';
        ctx.font = '700 32px Arial';
        ctx.fillText('Комментарий', 110, 875);
        ctx.fillStyle = '#303841';
        ctx.font = '400 31px Arial';
        wrapCanvasText(ctx, review.comment || '', 110, 925, 840, 44, 7);
        ctx.fillStyle = '#687583';
        ctx.font = '400 25px Arial';
        ctx.fillText('ID отзыва: ' + (review.id || 'не указан'), 110, 1290);
        ctx.fillText('Дата: ' + createdText, 110, 1332);
        ctx.fillText('Ссылка: https://moonn.ru/otzivi', 110, 1374);
        return canvas;
      }}
      ...
      function prepareReviewProof(review) {{
        var canvas = buildReviewProofImage(review);
        if(!canvas) return;
        var dataUrl = canvas.toDataURL('image/png');
        var reviewId = review && review.id ? String(review.id).replace(/[^a-zA-Z0-9_-]/g, '-') : 'review';
        var filename = 'moonn-review-proof-' + reviewId + '.png';
        if(proofPreview) {{
          proofPreview.src = dataUrl;
          proofPreview.hidden = false;
        }}
        if(proofCard) proofCard.classList.add('is-visible');
        if(shareProofButton && navigator.canShare && navigator.share && window.fetch && window.File) {{
          fetch(dataUrl).then(function(response) {{ return response.blob(); }}).then(function(blob) {{
            lastProofFile = new File([blob], filename, {{type: 'image/png'}});
            if(navigator.canShare({{files: [lastProofFile]}})) {{
              shareProofButton.hidden = false;
            }}
          }}).catch(function() {{}});
        }}
      }}
      ...
      if(copyOpenTelegramButton) {{
        copyOpenTelegramButton.addEventListener('click', function() {{
          copyText(offerMessageBox ? offerMessageBox.value : '', copyOpenTelegramButton);
          setTimeout(function() {{ window.open(telegramUrl, '_blank', 'noopener'); }}, 180);
        }});
      }}

      if(shareProofButton) {{
        shareProofButton.addEventListener('click', function() {{
          if(!lastProofFile || !navigator.share) return;
          var text = offerMessageBox ? offerMessageBox.value : 'Подтверждение отзыва Moonn.ru';
          copyText(text, shareProofButton);
          navigator.share({{
            title: 'Подтверждение отзыва Moonn',
            text: text,
            files: [lastProofFile]
          }}).catch(function() {{}});
        }});
      ...
      if(ratingLink) {{
        ...
        ratingLink.addEventListener('click', function(event){{
          ...
          ratingWasOpened = true;
          ratingLink.textContent = 'Оценка открыта в Яндекс Услугах';
          ratingLink.setAttribute('aria-disabled', 'true');
          ratingLink.classList.add('rf-disabled');
      ...
      if(form) {{
        ...
        function handleReviewSubmit(event) {{
          ...
          if(comment.length < 8) {{
            setStatus('error', 'Напишите чуть более подробный комментарий.');
            return;
          }}
          isSubmittingReview = true;
          if(submitButton) {{
            submitButton.disabled = true;
            submitButton.classList.add('rf-disabled');
          ...
          jsonp({{
            ...
            source: 'moonn_review_funnel'
          }}, function(err, payload) {{
            isSubmittingReview = false;
            if(submitButton) {{
              submitButton.disabled = false;
              submitButton.classList.remove('rf-disabled');
              submitButton.textContent = 'Отправить отзыв';
            }}
            if(err || !payload || !payload.ok) {{
              setStatus('error', 'Не удалось отправить отзыв. Попробуйте ещё раз.');
              return;
            }}
            form.reset();
              setStatus('ok', 'Отзыв опубликован на странице Татьяна Мун.ру.');
...
def main() -> None:
    ...
    OTZIVI_T123_COMBINED_PATH.write_text(block + "\n" + YCLIENTS_WIDGET_SNIPPET + "\n", encoding="utf-8")
